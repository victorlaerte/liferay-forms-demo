...
int status = ParamUtil.getInteger(request, "status", WorkflowConstants.STATUS_ANY);
      ...
      _jspx_th_aui_a_0.setHref( themeDisplay.getPathMain() + "/bookmarks/open_entry?entryId=" + entry.getEntryId() + ((status != WorkflowConstants.STATUS_ANY) ? "&status=" + status : StringPool.BLANK) );
      _jspx_th_aui_a_0.setTarget("_blank");
      int _jspx_eval_aui_a_0 = _jspx_th_aui_a_0.doStartTag();
      if (_jspx_eval_aui_a_0 != javax.servlet.jsp.tagext.Tag.SKIP_BODY) {
        out.print( HtmlUtil.escape(entry.getName()) );
        out.write(' ');
        out.write('(');
        out.print( HtmlUtil.escape(entry.getUrl()) );
        out.write(')');
      }
      ...
      out.print( HtmlUtil.escape(entry.getDescription()) );
      out.write("</p>\n\n");
